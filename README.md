...
- **Live canvas + history via Trigger.dev Realtime** — while a workflow runs, the canvas opens a single `useRealtimeRunsWithTag('wfrun:<id>')` SSE subscription using a server-minted `publicAccessToken`. The same stream drives **everything**: the per-node pulse/glow (mapping each `node-runner` run's status onto the canvas `runStatus` map via `nodeId:<canvasId>` + `kind:<type>` tags), the History sidebar timeline, the Gemini token-by-token streaming, the per-node outputs (Gemini text / Crop CDN URL / Response per-edge map are read straight off `run.output` — `nodeRunnerTask.run` returns the worker's output so Trigger ships it via SSE), and the workflow-terminal signal (orchestrator COMPLETED + every node-runner terminal, with a 1.5 s debounce, fires a single `onWorkflowComplete` to flip `setRunning(false)`). The 5 s `pollRun` loop in `WorkflowCanvas.tsx` is now invoked only when the public-access token mint fails — true fallback, off in the happy path.
...
- **Live Gemini streaming** — Gemini's response renders token-by-token in both the Gemini node and any downstream Response cards, instead of jumping from "No output yet" to the full text at the end. The worker pipes `model.generateContentStream()` through Trigger.dev's [`streams.pipe("gemini", …)`](https://trigger.dev/docs/realtime/overview#how-do-i-stream-ai%2Fllm-responses-from-a-background-task), and the browser subscribes via `useRealtimeRunWithStreams` over SSE. If the public-access token mint fails (rare), the 5 s `pollRun` fallback still surfaces the final text once the run completes — chunks won't animate, but the response still lands.
...
- **Image cropping runs ffmpeg inside the Trigger.dev worker (not Transloadit's image robot).** The `ffmpeg()` build extension from `@trigger.dev/build/extensions/core` apt-installs ffmpeg into the deployed worker image and exports `FFMPEG_PATH=/usr/bin/ffmpeg`. `src/lib/ffmpegCrop.ts` is a single ffmpeg invocation: download input → `ffmpeg -i in.jpg -vf "crop=iw*W/100:ih*H/100:iw*X/100:ih*Y/100" -frames:v 1 out.jpg` → upload via Transloadit's `/upload/handle`. The `crop` filter is a libavfilter video filter, but ffmpeg's `image2` demuxer + `mjpeg` encoder treat a JPG as a 1-frame stream, so the same filter operates on stills with no video container in the loop — one process spawn, one mjpeg encode, no lossy intermediate codec. Earlier attempts to use Transloadit's `/video/encode` robot for an in-platform "FFmpeg crop" tripped `VIDEO_ENCODE_VALIDATION` on image inputs — running ffmpeg ourselves sidesteps that entirely.
- **Mandatory 30 s artificial delay on Crop** is preserved (PRD requirement: "at least 30 s"). `runCropImage` runs ffmpeg → buffer, then `Promise.all`s the Transloadit upload alongside a 30 s `setTimeout` — total wall-clock is `max(30 s, upload)` ≈ 32 s, not `30 s + upload` ≈ 40 s. The PRD specifies the floor, not when the timer starts. Upload failure rejects fast so FAILED rows aren't held up by the dangling sleep. SUCCESS-guard at the top of the worker still skips both phases on retry.
...
- **One realtime coordinator drives canvas pulse, node outputs, terminal detection, and Gemini token streaming.** `<RealtimeCoordinator>` in `src/components/canvas/RealtimeCoordinator.tsx` is the only realtime entry point. It mounts while a run is in flight, fetches a `wfrun:<id>`-scoped `publicAccessToken` from `/api/runs/[runId]/realtime-token`, and opens a single `useRealtimeRunsWithTag('wfrun:<id>')` SSE subscription. From that one stream it does four jobs: (a) maps every `node-runner` run's status to a `runStatus` entry keyed by canvas nodeId (the per-node pulse/glow); (b) reads each completed run's `run.output` and writes it onto canvas state via `updateNodeData` (Gemini `response`, Crop `outputUrl`, Response `result`/`results`) — possible because `nodeRunnerTask.run` now returns the worker's output instead of `void`, and Trigger.dev serializes that as the run's `output` over SSE; (c) for every `kind:gemini` run it mounts a child subscriber on `useRealtimeRunWithStreams(runId)` to consume `streams.pipe("gemini", streamGeminiText(...))` chunks emitted by `runGemini` on the worker (token-by-token text lands in a `streamingText: Record<canvasNodeId, string>` slice that the Gemini node, Response cards, and History sidebar all prefer over the persisted `data.response` while non-empty); (d) detects workflow-level terminal — orchestrator (`run-workflow`) status COMPLETED **and** every `node-runner` run in a terminal Trigger status, with a 1.5 s debounce to absorb late cascades — and fires `onWorkflowComplete` exactly once to flip `setRunning(false)` + bump the history-key. The `pollRun` loop in `WorkflowCanvas.tsx` is **only invoked when the public-access token mint fails** (i.e., realtime is provably unavailable for this run). In the happy path no polling happens at all.
