...
- **Gemini responses stream over Trigger.dev Realtime, not over a custom SSE route.** `runGemini` calls `streams.pipe("gemini", streamGeminiText(...))` from `@trigger.dev/sdk/v3`, attaching the chunk iterable to the active `node-runner` run. Each `nodeRunnerTask.trigger(...)` is tagged with `wfrun:<id>`, `nodeId:<canvasId>`, and `kind:<type>` so the browser can find the right run from the canvas. `/api/runs/[runId]/realtime-token` mints a `wfrun:<id>`-scoped public access token; `<GeminiStreamCoordinator>` uses it with `useRealtimeRunsWithTag` to discover gemini runs and `useRealtimeRunWithStreams` to subscribe to each one's chunks. Accumulated text is written to a `streamingText: Record<canvasNodeId, string>` slice on the Zustand store; the Gemini node and Response cards prefer it over the persisted `data.response` while it's non-empty. The 2 s `pollRun` is kept as a fallback so a token/SSE failure still surfaces the final text.
