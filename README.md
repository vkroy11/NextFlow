...
- **Live canvas + history via Trigger.dev Realtime** — while a workflow runs, the canvas opens a single `useRealtimeRunsWithTag('wfrun:<id>')` SSE subscription using a server-minted `publicAccessToken`. The same stream drives both the History sidebar timeline and the per-node pulse/glow on the canvas (each `node-runner` run carries `nodeId:<canvasId>` + `kind:<type>` tags, so the coordinator maps run statuses straight onto the canvas `runStatus` map). A slow 5 s `pollRun` loop on `/api/runs/[runId]` is kept as a true fallback: it backfills the same status if the SSE stream errors or the token mint fails, and is also responsible for reading each NodeRun's persisted `output` once it succeeds (Gemini text, Crop CDN URL, Response per-edge map) onto canvas state.
...
- **One realtime coordinator drives both canvas pulse and Gemini token streaming.** `<RealtimeCoordinator>` in `src/components/canvas/RealtimeCoordinator.tsx` is the only realtime entry point. It mounts while a run is in flight, fetches a `wfrun:<id>`-scoped `publicAccessToken` from `/api/runs/[runId]/realtime-token`, and opens a single `useRealtimeRunsWithTag('wfrun:<id>')` SSE subscription. From that one stream it does two jobs: (a) maps every `node-runner` run's status to a `runStatus` entry keyed by canvas nodeId (driving the per-node pulse/glow), and (b) for every `kind:gemini` run it mounts a child subscriber on `useRealtimeRunWithStreams(runId)` to consume `streams.pipe("gemini", streamGeminiText(...))` chunks emitted by `runGemini` on the worker. Token-by-token text lands in a `streamingText: Record<canvasNodeId, string>` slice; the Gemini node, Response cards, and History sidebar all prefer it over the persisted `data.response` while it's non-empty. The slow 5 s `pollRun` loop in `WorkflowCanvas.tsx` is the true fallback path — it still updates `runStatus` on its own (idempotent with the realtime push) and reads each NodeRun's persisted `output` onto canvas state.
